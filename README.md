...
작업지시부터 생산실적, 불량, 중단/재가동, ERP 연동, AI 기반 품질 자동 판정까지 포함한 MES 통합 설계 문서입니다.
...
| 주요 연동 | MES ↔ ERP, MES ↔ AI, PostgreSQL, Redis(선택) |
...
`ERP 작업지시 생성 → MES 작업지시 수신 → 작업 시작 → 생산 진행 → 불량 발생 / 작업 중단 / 재가동 → 작업 종료 → 일일생산코드(DP) 생성 → 품질 샘플 채취 → 미생물 실험 이미지 촬영 → AI 분석 수행 → PASS / FAIL 도출 → quality_test.result 반영 → production.status 반영 → 생산실적 / 품질결과 집계 → ERP 저장`
...
| AI 품질 판정 | 이미지 업로드, 모델 추론, `PASS` / `FAIL` 반환 |
...
| 생산 실행 | 작업 시작 후 생산량, 불량, 중단 이력 관리 |
| 생산 종료 | 생산 완료 후 생산 실적 및 일일생산코드 생성 |
| 품질 검사 | 샘플 채취 후 품질 검사 대상 생성 |
| AI 분석 | 실험 이미지를 AI 모델에 전달 후 판정 결과 수신 |
...
| 품질 검사 화면 | 품질번호, 일일생산코드, 검사결과, 검사시각, 이미지 업로드 |
...
| --- | --- |
| 이미지 업로드 | 실험 이미지를 업로드 |
| 모델 추론 | `CNN`, `ResNet`, `YOLO` 등으로 분석 수행 |
| 결과 반환 | `PASS` / `FAIL` 반환 |
| MES API 호출 | 품질 결과를 MES에 반영 |

가정: 실제 이미지 분석 모델이 결과를 반환하고, MES는 그 이후 판정 결과 반영과 상태 전이를 담당합니다.
...
`품질번호(QT) 바코드 생성 → 현장 스캔 → 품질 데이터 조회 → 생산 데이터 자동 연결`
...
AI가 품질을 판정하고 MES가 생산과 품질 상태를 자동으로 연결해 관리하는 스마트팩토리형 통합 구조입니다.

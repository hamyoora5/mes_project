...
`ERP 작업지시 생성 → MES 작업지시 수신 → 작업 시작 → 생산 진행 → 불량 발생 / 작업 중단 / 재가동 → 작업 종료 → 일일생산코드(DP) 생성 → 실험번호(QT) 생성 및 바코드 라벨 부착 → 실험물 관리 → 가상 AI 판정(CNN / ResNet / YOLO 등 가정) → 적합 / 부적합 결과 도출 → quality_test.result 반영 → production.status 반영 → 생산실적 / 품질결과 집계 → ERP 저장`
